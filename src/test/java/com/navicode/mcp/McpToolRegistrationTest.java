...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("navicode.audit.dir");
        System.setProperty("navicode.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("navicode.audit.dir");
            } else {
                System.setProperty("navicode.audit.dir", previous);
            }
        }
    }
