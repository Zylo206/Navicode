...
class MemoryRetrieverTest {
    ...
    @Test
    void promptContextDefaultsToLongTermOnly() {
        shortTerm.store(new MemoryEntry("s1", "当前会话提到 Maven 构建",
                MemoryEntry.MemoryType.SUMMARY, null, 10));
        longTerm.store(new MemoryEntry("f1", "项目使用 Maven 构建",
                MemoryEntry.MemoryType.FACT, null, 10));

        String context = retriever.buildPromptMemoryContextForQuery(
                "Maven",
                200,
                null,
                MemoryRetrievalMode.LONG_TERM_ONLY,
                java.util.List.of());

        assertTrue(context.contains("相关长期记忆"));
        assertTrue(context.contains("项目使用 Maven 构建"));
        assertFalse(context.contains("相关短期上下文"));
        assertFalse(context.contains("当前会话提到 Maven 构建"));
    }

    @Test
    void experimentalPromptContextIncludesEligibleShortTermSummary() {
        shortTerm.store(new MemoryEntry("s1", "历史摘要：当前任务使用 Maven 构建",
                MemoryEntry.MemoryType.SUMMARY, null, 10));
        longTerm.store(new MemoryEntry("f1", "项目偏好使用 Java",
                MemoryEntry.MemoryType.FACT, null, 10));
        ...
        String context = retriever.buildPromptMemoryContextForQuery(
                ...
                200,
                null,
                MemoryRetrievalMode.LONG_PLUS_SHORT,
                java.util.List.of());

        assertTrue(context.contains("相关短期上下文（实验）"));
        assertTrue(context.contains("历史摘要：当前任务使用 Maven 构建"));
    }

    @Test
    void experimentalPromptContextSkipsCurrentQueryFromShortTerm() {
        shortTerm.store(new MemoryEntry("u1", "请用 Maven 构建项目",
                MemoryEntry.MemoryType.CONVERSATION, null, 10));
        ...
        assertFalse(context.contains("请用 Maven 构建项目"));
    }

    @Test
    void experimentalPromptContextSkipsActiveHistoryShortTerm() {
        shortTerm.store(new MemoryEntry("tool1", "[read_file] Maven 配置如下",
                MemoryEntry.MemoryType.TOOL_RESULT, null, 10));
        ...
        String context = retriever.buildPromptMemoryContextForQuery(
                ...
                null,
                MemoryRetrievalMode.LONG_PLUS_SHORT,
                java.util.List.of("用户请求\n[read_file] Maven 配置如下\n助手已处理"));

        assertFalse(context.contains("[read_file] Maven 配置如下"));
    }

    @Test
    void experimentalShortTermContextUsesAtMostThirtyPercentBudget() {
        shortTerm.store(new MemoryEntry("s1", "Maven 摘要 1",
                MemoryEntry.MemoryType.SUMMARY, null, 20));
        shortTerm.store(new MemoryEntry("s2", "Maven 摘要 2",
                MemoryEntry.MemoryType.SUMMARY, null, 20));
        ...
        int included = (context.contains("Maven 摘要 1") ? 1 : 0)
                + (context.contains("Maven 摘要 2") ? 1 : 0);
