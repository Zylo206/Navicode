...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("navicode.snapshot.enabled", "NAVICODE_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("navicode.snapshot.dir", "NAVICODE_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".navicode", "snapshots").toString()));
        int max = readInt("navicode.snapshot.max", "NAVICODE_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("navicode.snapshot.excludes", "NAVICODE_SNAPSHOT_EXCLUDES", ""));
