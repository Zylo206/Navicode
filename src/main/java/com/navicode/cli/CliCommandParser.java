...
final class CliCommandParser {
    ...
    static ParsedCommand parse(String input) {
        ...
        if (trimmed.equalsIgnoreCase("/goal")) {
            return new ParsedCommand(CommandType.GOAL, null);
        }

        if (trimmed.regionMatches(true, 0, "/goal ", 0, 6)) {
            return new ParsedCommand(CommandType.GOAL, trimmed.substring(6).trim());
        }

        if (trimmed.equalsIgnoreCase("/dream")) {
            return new ParsedCommand(CommandType.DREAM, null);
        }
