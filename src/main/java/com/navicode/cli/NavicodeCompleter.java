...
import com.navicode.mcp.mention.AtMentionCompleter;
...
final class NavicodeCompleter implements Completer {
    private final Supplier<List<McpResourceDescriptor>> resourceSupplier;
    private final Supplier<List<Skill>> skillSupplier;

    NavicodeCompleter(Supplier<List<McpResourceDescriptor>> resourceSupplier) {
    ...
    private boolean completeTask(String input, List<Candidate> candidates) {
        ...
        addMatching(candidates, "后台任务", payload,
                ...
                option("new ", "创建树状工作任务"),
                option("sub ", "创建子工作任务"),
                option("board", "查看工作任务板"),
                option("start ", "标记工作任务进行中"),
                option("block ", "标记工作任务阻塞"),
                option("done ", "标记工作任务完成"),
                option("abandon ", "放弃工作任务"),
                option("gate", "检查未完成任务闸门"));
        return true;
    }
