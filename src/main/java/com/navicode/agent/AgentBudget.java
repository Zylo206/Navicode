 ...
 * 1. 系统属性：{@code navicode.react.token.budget} / {@code navicode.react.stagnation.window} /
 *    {@code navicode.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dnavicode.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dnavicode.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("navicode.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("navicode.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("navicode.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
