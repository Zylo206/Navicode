...
import com.navicode.llm.LlmTraceLogger;
import com.navicode.context.ContextProfile;
import com.navicode.context.TokenUsageFormatter;
import com.navicode.lsp.LspDiagnosticReport;
import com.navicode.memory.ConversationHistoryCompactor;
import com.navicode.memory.ExplicitMemoryHints;
import com.navicode.memory.MemoryManager;
import com.navicode.prompt.PromptAssembler;
import com.navicode.prompt.PromptContext;
import com.navicode.prompt.PromptMode;
import com.navicode.render.PlainRenderer;
import com.navicode.render.Renderer;
import com.navicode.render.StatusInfo;
import com.navicode.runtime.CancellationContext;
import com.navicode.skill.SkillContextBuffer;
import com.navicode.skill.SkillIndexFormatter;
import com.navicode.skill.SkillRegistry;
import com.navicode.util.AnsiStyle;
import com.navicode.tool.ToolRegistry;
import com.navicode.tool.ToolRegistry.ToolExecutionResult;
import com.navicode.tool.ToolRegistry.ToolInvocation;
import com.navicode.util.TerminalMarkdownRenderer;
import com.navicode.image.ImageReferenceParser;
...
public class Agent {
    ...
    public String run(String userInput) {
        ...
        // 检索 prompt 记忆上下文。默认只注入长期记忆；实验模式会用 active history 去重短期上下文。
        ContextProfile contextProfile = memoryManager.getContextProfile();
        String memoryContext = memoryManager.buildPromptMemoryContextForQuery(
                userInput,
                contextProfile.memoryContextTokens(),
                activeMessageTexts());
        ...
        while (true) {
            ...
            try {
                ...
                memoryManager.updateWorkspaceCheckpoint(userInput, response.content());
                String goalJudgeMessage = evaluateGoal(userInput, response.content());
                ...
                if (streamRenderer.hasStreamedOutput()) {
                    if (!goalJudgeMessage.isBlank()) {
                        renderer().stream().println(goalJudgeMessage);
                    }
                    streamRenderer.finish();
                    return "";
                }
                streamRenderer.clearThinkingPanel();
                String formatted = formatUserFacingResponse(reasoningTranscript.toString(), response.content());
                return appendGoalJudgeMessage(formatted, goalJudgeMessage);
    ...
    private List<String> activeMessageTexts() {
    ...
    private String evaluateGoal(String userInput, String assistantOutput) {
        if (memoryManager.getGoalManager() == null) {
            return "";
        }
        return memoryManager.getGoalManager()
                .judgeCompletion(llmClient, userInput, assistantOutput)
                .userMessage();
    }

    private static String appendGoalJudgeMessage(String response, String goalJudgeMessage) {
        if (goalJudgeMessage == null || goalJudgeMessage.isBlank()) {
            return response;
        }
        if (response == null || response.isBlank()) {
            return goalJudgeMessage;
        }
        return response + "\n\n" + goalJudgeMessage;
    }
    ...
    public String getContextStatus() {
        com.navicode.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.navicode.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.navicode.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.navicode.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.navicode.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.navicode.memory.MemoryEntry.estimateTokens(
