...
public class MemoryRetriever {
    private static final double SHORT_TERM_CONTEXT_RATIO = 0.30;
    ...
    public String buildContextForQuery(String query, int maxTokens) {
        return buildPromptMemoryContextForQuery(query, maxTokens, null, MemoryRetrievalMode.LONG_TERM_ONLY, List.of());
    }

    public String buildContextForQuery(String query, int maxTokens, String projectKey) {
        return buildPromptMemoryContextForQuery(query, maxTokens, projectKey, MemoryRetrievalMode.LONG_TERM_ONLY, List.of());
    }

    public String buildPromptMemoryContextForQuery(String query,
                                                   int maxTokens,
                                                   String projectKey,
                                                   MemoryRetrievalMode mode,
                                                   Collection<String> activeHistoryTexts) {
        MemoryRetrievalMode effectiveMode = mode == null ? MemoryRetrievalMode.LONG_TERM_ONLY : mode;
        int shortBudget = effectiveMode.includesShortTerm()
                ? Math.max(0, (int) Math.floor(maxTokens * SHORT_TERM_CONTEXT_RATIO))
                : 0;
        int longBudget = Math.max(0, maxTokens - shortBudget);

        String longContext = buildLongTermContext(query, longBudget, projectKey);
        String shortContext = effectiveMode.includesShortTerm()
                ? buildShortTermContext(query, shortBudget, activeHistoryTexts)
                : "";
    ...
    private String buildLongTermContext(String query, int maxTokens, String projectKey) {
    ...
    private String buildShortTermContext(String query, int maxTokens, Collection<String> activeHistoryTexts) {
        if (maxTokens <= 0) {
            return "";
        }

        Set<String> active = normalizedTexts(activeHistoryTexts);
        List<ScoredEntry> scored = new ArrayList<>();
        for (MemoryEntry entry : shortTermMemory.getAll()) {
            if (isDuplicateShortTermEntry(entry, query, active)) {
                continue;
            }
            double score = computeRelevanceScore(entry, query);
            if (score > 0) {
        ...
        List<MemoryEntry> relevant = scored.stream()
                .sorted(Comparator
                        .comparing((ScoredEntry scoredEntry) -> scoredEntry.entry().getType() == MemoryEntry.MemoryType.SUMMARY)
                        .reversed()
                        .thenComparing(Comparator.comparingDouble(ScoredEntry::score).reversed()))
                .map(ScoredEntry::entry)
                .collect(Collectors.toList());
        if (relevant.isEmpty()) {
            return "";
    ...
    private static boolean isDuplicateShortTermEntry(MemoryEntry entry, String query, Set<String> activeHistoryTexts) {
        String content = normalizeForDuplicateCheck(entry.getContent());
        if (content.isBlank()) {
            return true;
        }
        String normalizedQuery = normalizeForDuplicateCheck(query);
        if (!normalizedQuery.isBlank() && content.equals(normalizedQuery)) {
            return true;
        }
        for (String active : activeHistoryTexts) {
            if (active.equals(content) || active.contains(content)) {
                return true;
            }
        }
        return false;
    ...
    private static Set<String> normalizedTexts(Collection<String> texts) {
        if (texts == null || texts.isEmpty()) {
            return Set.of();
        }
        return texts.stream()
                .map(MemoryRetriever::normalizeForDuplicateCheck)
