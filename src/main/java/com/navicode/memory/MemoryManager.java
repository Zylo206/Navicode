...
public class MemoryManager {
    ...
    private final MemoryRetrievalMode retrievalMode;
    ...
    public MemoryManager(LlmClient llmClient,
                         ...
                         MemoryRetrievalMode retrievalMode) {
        this(llmClient, ContextProfile.custom(contextWindow, shortTermBudget), longTermMemory, retrievalMode);
    }

    private MemoryManager(LlmClient llmClient, ContextProfile contextProfile, LongTermMemory longTermMemory) {
        this(llmClient, contextProfile, longTermMemory, MemoryRetrievalMode.configured());
    }
    ...
    private MemoryManager(LlmClient llmClient,
        ...
        this.retrievalMode = retrievalMode == null ? MemoryRetrievalMode.LONG_TERM_ONLY : retrievalMode;
        this.tokenBudget = new TokenBudget(contextProfile.maxContextWindow());
        this.currentProject = defaultProjectKey();
        this.workspaceMemory = WorkspaceMemory.forProject(this.currentProject);
        this.goalManager = new GoalManager(this.workspaceMemory);
    }
    ...
    public void storeFact(String fact, String scope) {
        ...
        if ("project".equals(normalizedScope) && workspaceMemory != null) {
            workspaceMemory.appendProjectFact(fact);
        }
    }
    ...
    public String buildContextForQuery(String query, int maxTokens) {
        return buildPromptMemoryContextForQuery(query, maxTokens, List.of());
    }

    public String buildContextForQuery(String query, int maxTokens, Collection<String> activeHistoryTexts) {
        return buildPromptMemoryContextForQuery(query, maxTokens, activeHistoryTexts);
    }

    public String buildPromptMemoryContextForQuery(String query,
                                                   int maxTokens,
                                                   Collection<String> activeHistoryTexts) {
        String longTermContext = retriever.buildPromptMemoryContextForQuery(
                query,
                maxTokens,
                currentProject,
                retrievalMode,
                activeHistoryTexts);
        String workspaceContext = workspaceMemory == null
                ? ""
                : workspaceMemory.buildPromptContext(Math.max(500, maxTokens / 2));
    ...
    public void updateWorkspaceCheckpoint(String userInput, String assistantOutput) {
        if (workspaceMemory != null) {
            workspaceMemory.updateCheckpoint(userInput, assistantOutput);
        }
    }
    ...
    public String buildDreamSuggestionDraft() {
        return workspaceMemory == null ? "" : workspaceMemory.buildDreamSuggestionDraft();
    }
    ...
    public boolean compressIfNeeded() {
        if (!retrievalMode.includesShortTerm()) {
            return false;
        }
        // 默认短期上下文由 conversationHistory 承担；只有实验短期检索模式才需要压缩 shortTermMemory。
    ...
    public String getSystemStatus() {
        return "上下文策略: " + contextProfile.summary() + "\n" +
                "记忆检索模式: " + retrievalMode.displayName() + "\n" +
                shortTermMemory.getStatusSummary() + "\n" +
                longTermMemory.getStatusSummary() + "\n" +
                (workspaceMemory == null ? "工作区记忆: 未启用" : workspaceMemory.statusLine()) + "\n" +
    ...
    public MemoryRetrievalMode getRetrievalMode() { return retrievalMode; }
