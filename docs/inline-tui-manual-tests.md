...
```bash
cd /Users/itwanger/Documents/GitHub/navicode
mvn -q clean package -DskipTests
```

产物：`target/navicode-1.0-SNAPSHOT.jar`，所有用例都基于这个 jar 启动。
...
**预期**：
- Banner 显示简洁的 `Navicode v16.1.0` 开屏，不带右侧盒线边框
...
```bash
NAVICODE_RENDERER=lanterna java -jar target/navicode-1.0-SNAPSHOT.jar
```
...
## 11. 形态切换 — 兼容旧 NAVICODE_TUI（用例 10）
...
**预期**：
- stderr 出现 `⚠️ NAVICODE_TUI=true 已废弃，请改用 NAVICODE_RENDERER=lanterna`
...
```bash
NAVICODE_RENDERER=plain java -jar target/navicode-1.0-SNAPSHOT.jar
```
...
```bash
NO_COLOR=1 java -jar target/navicode-1.0-SNAPSHOT.jar
```
...
```bash
NAVICODE_RENDERER=weird java -jar target/navicode-1.0-SNAPSHOT.jar
```

**预期**：stderr `⚠️ 未识别的 NAVICODE_RENDERER='weird'，回退到 inline`，正常进 inline 模式。
...
```bash
TERM=dumb java -jar target/navicode-1.0-SNAPSHOT.jar
```
...
```
*
 Navicode  idle  glm-5.1  ctx 0/200.0k  HITL OFF  MCP 4/4  Skill 2/2
...
**输入**：
```
把 ROADMAP.md 第一行改成 "# Navicode 路线图（v16.1 测试）"
```
...
📝 ROADMAP.md
@@ -1,N +1,N @@
- # Navicode ROADMAP（原标题）
+ # Navicode 路线图（v16.1 测试）
  ...（上下文行）
```
...
│   [6] 配置文件: ~/.navicode/config.json (只读视图，编辑请用编辑器)
...
## 23. NAVICODE_NO_STATUSBAR 禁用状态栏

```bash
NAVICODE_NO_STATUSBAR=true java -jar target/navicode-1.0-SNAPSHOT.jar
```
...
- 旧 Windows cmd / PuTTY 等不适合 ANSI 光标控制的终端：用 `NAVICODE_NO_STATUSBAR=true` 即可
